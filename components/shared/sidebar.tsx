    ...
    <div className="relative">
      <Button
        ...
        className={`h-8 w-8 hover:bg-muted absolute ${
          !isCollapsed
            ...
            : "top-8 left-8 transform -translate-x-1/2 -translate-y-1/2 border rounded-full p-0.5 z-99 bg-white"
        }`}
      ...
      >
        ...
        <div className="border-b px-2 p-2 relative">
          ...
          >
            {isCollapsed ? (
              <ChevronRight className="h-4 w-4 absolute left-0" />
            ) : (
              <ChevronLeft className="h-4 w-4" />
            )}
          </Button>
      ...
      {!isCollapsed && (
        <div className="fixed inset-0 z-5 backdrop-blur-xs backdrop-grayscale bg-black/20" />
      )}
    </div>
  ) : (
    <div
